      ...
      <form @submit.prevent="handleSubmit" class="space-y-4 text-black">
        ...
        <div>
          <label for="dates" class="block text-sm font-medium text-gray-700"
            >Dates (Select at least one)</label
          >
          <input
            type="date"
            id="dates"
        ...
        <div>
          <label
            ...
            >Tournament Contact Email</label
          >
          <input
            type="email"
            id="contactEmail"
        ...
        <div>
          <label for="ageGroups" class="block text-sm font-medium text-gray-700"
            >Age Groups</label
          >
          <div class="flex flex-wrap mt-1">
            <div
              v-for="ageGroup in VALID_AGE_GROUPS"
              :key="ageGroup"
              class="flex items-center mr-4 mb-2"
            >
              <input
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full">
        ...
        <div class="mt-4">
          <button
            @click="closeModal"
            class="bg-sky-700 hover:bg-sky-500 text-white py-2 px-4 rounded-full"
          >
            Close
          </button>
        </div>
    ...
    async handleSubmit() {
      try {
        const response = await fetch(
          'http://localhost:5000/api/v1/tournament-submissions',
          {
            method: 'POST',
            headers: {
